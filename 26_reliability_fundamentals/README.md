...
3. **Delivery Semantics** — два сценария:
   ...
   - *at-least-once*: воркер применяет side effect, но падает **до отправки ACK** →
     мастер не получает подтверждения → переотправляет по visibility timeout →
     тот же side effect применяется **второй раз** (двойной эффект — цена
     at-least-once; счётчик в shared memory показывает `applied 2 times`).
